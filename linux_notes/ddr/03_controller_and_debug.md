...
> **对应手册**：Â§35.1 Overview、Â§35.1.1 MMDC feature summary
...
> **对应手册**：Â§35.4 Functional Description

#### 12.4.1 写/读数据流

> **对应手册**：Â§35.4.1 Write/Read data flow
...
> **对应手册**：Â§35.4.2 MMDC initialization

MMDC 在芯片复位后处于禁用状态，无时钟输出到 DDR 设备。正确的激活需要以下步骤：

1. 设置 `MDSCR[CON_REQ]`，发出配置请求（因为 MMDC 刚复位处于禁用状态，无需轮询 `CON_ACK`）
2. 在 `MDCFG0`、`MDCFG1`、`MDOTC`、`MDCFG2` 寄存器中配置时序参数
3. 在 `MDMISC` 寄存器中配置 DDR 类型和其他杂项参数
4. 在 `MDOR` 寄存器中配置复位退出延迟
5. 在 `MDCTL` 寄存器中配置 DDR 物理参数（密度和突发长度）
6. 执行 ZQ 校准，正确初始化驱动强度
7. 通过 `MDCTL[SDE_0]`（片选0）和 `MDCTL[SDE_1]`（片选1）使能 MMDC。此时 MMDC 开始 JEDEC 定义的 DDR_RESET/DDR_SDCKE 复位和初始化序列
8. 通过 `MDSCR` 寄存器发出 MRS/MRW 命令（ZQ、ODT、PRE 等），完成 JEDEC 定义的初始化序列

> **注意**：为保证 DRAM_RESET 和 DRAM_SDCKE 信号在芯片上电和复位期间保持低电平（JEDEC 要求），必须将它们连接到下拉电阻。


#### 12.4.3 MMDC 寄存器配置

> **对应手册**：Â§35.4.3 Configuring the MMDC registers
...
> **对应手册**：Â§35.4.4 MMDC Address Space
...
#### 12.4.4.4 地址镜像（Address Mirroring）
...
#### 12.4.5 LPDDR2/DDR3 引脚复用映射

> **对应手册**：Â§35.4.5 LPDDR2 and DDR3 pin mux mapping

MMDC 支持 LPDDR2 和 DDR3 两种模式，不同类型的 DDR 使用不同的引脚分配方案。在 i.MX 6ULL 中，引脚复用由 IOMUXC 模块配置，MMDC 根据 `MDMISC` 寄存器中的 DDR 类型设置来选择正确的引脚映射。

DDR3 模式下使用完整的地址/命令/数据引脚组（ADDR、CAS、RAS、WE、CS、CKE、BA 等），而 LPDDR2 模式下部分引脚功能会重新定义。具体的引脚分配需要参考 i.MX 6ULL 参考手册的 IOMUX 章节。


#### 12.4.6 省电与时钟频率切换

> **对应手册**：§35.4.6 Power Saving and Clock Frequency Change modes


#### 12.4.6.1 省电概述

> **对应手册**：§35.4.6.1 Power saving general

MMDC 支持多种 DDR 省电模式（默认禁用），可显著降低功耗：

**1. Self-Refresh（自刷新）入口激活** — 对整个 DDR 设备（两个片选）：

| 机制 | 说明 |
|------|------|
| LPMD（Low Power Mode） | 用于**省电目的**。硬件握手（LPMD/LPACK）或软件握手（`MAPSR[LPMD]` + `MAPSR[LPACK]`）或自动进入（`MAPSR[PST]` 配置空闲周期） |
| DVFS（Dynamic Voltage and Frequency Change） | 用于**时钟频率变化**。硬件握手（DVFS/DVACK）或软件握手（`MAPSR[DVFS]` + `MAPSR[DVACK]`） |

> **LPMD vs DVFS 的区别**：两者进入/退出流程相同，但目的不同。LPMD 纯粹为了省电，DVFS 是为了改变 DDR 工作频率。SR 期间不需要发周期性刷新命令。

**2. Power Down（掉电模式）自动入口** — 针对特定片选，通过 `ESDPDC` 寄存器配置：
...
- `PWDT_0`/`PWDT_1`：定义每个片选进入 Power Down 前的空闲周期数，可分别配置
- `SLOW_PD`：DDR3 使用慢速预充电掉电时需设置
- `BOTH_CS_PS`：可独立让每个片选进入 Power Down，或要求两个片选都空闲才一起进入

**3. 自动预充电** — 通过 `ESDPDC` 的 `PRCT_0`/`PRCT_1` 字段，可配置每个片选在空闲 N 个周期后自动预充电所有 Bank（2 ~ 128 个周期）。

> **注意**：进入省电模式还需配置时序参数 `MDCFG0[tXP, tXPDLL]` 和 ODT 时序 `MDOTC[tAOFPD, tAONPD, tANPD, tAXPD]`。


#### 12.4.6.2 自刷新与频率切换进出流程

> **对应手册**：§35.4.6.2 Self refresh and Frequency change entry/exit

MMDC 对 LPMD/DVFS 的硬件/软件握手处理方式相同：
...
> **对应手册**：Â§35.4.9 Burst Length options towards DDR
...
> **对应手册**：Â§35.4.10 Exclusive accesses handling
...
> **对应手册**：Â§35.4.11 AXI Error Handling
...
> **对应手册**：Â§35.5 Performance

#### 12.5.1 仲裁与重排序机制

> **对应手册**：Â§35.5.1 Arbitration and reordering mechanism

#### 12.5.1.1 仲裁流程（Arbitration General）
...
> **对应手册**：Â§35.5.2 Prediction mechanism
...
> **对应手册**：Â§35.5.3 Special Optimization for accesses towards DDR3
...
> **[补充内容] 非对齐 WRAP 访问优化**：当 AXI wrap 突发非对齐但**其 wrap 边界与 DDR wrap 边界一致**时，MMDC 会优化为只发一次 DDR 访问（而不是拆分多次），内部进行数据重排。
...
> **对应手册**：Â§35.6 MMDC Debug

#### 12.6.1 硬件调试监视器

> **对应手册**：Â§35.6.1 Hardware debug monitor
...
> **对应手册**：Â§35.6.2 Step By Step (SBS) software monitor
...
### 12.8 LPDDR2 刷新率更新与时序去加重

> **对应手册**：Â§35.8 LPDDR2 Refresh Rate Update and Timing Derating

LPDDR2 设备可能内置温度传感器，用于确定合适的刷新速率以及是否需要进行 AC 时序去加重（Timing Derating）。温度传感器的状态可通过 MRR 命令从 LPDDR2 的 MR4 寄存器读取。

MMDC 支持在线刷新更新和时序去加重机制，操作步骤如下：

1. **周期性轮询 MR4**：使用 MRR 命令读取 LPDDR2 MR4 寄存器
2. **读取 MDMRR 寄存器**：分析 MR4 指示的温度状态
3. **更新参数**：如果需要刷新率更新和/或 AC 时序去加重，则更新 `MDREF` 和/或 `MDMR4[tRCD_DE, tRC_DE, tRAS_DE, tRP_DE, tRRD_DE]` 参数

> **注意**：`MDMR4` 中的去加重值参考 `MDCFG3LP[tRC_LP, tRP_LP, tRCD_LP]`、`MDCFG1[tRAS]`、`MDCFG2[tRRD]` 中配置的基础时序参数。此功能仅适用于 LPDDR2，DDR3 不涉及。


### 12.9 DLL 切换

> **对应手册**：Â§35.9 DLL Switching
...
> **对应手册**：Â§35.10 ODT Configuration
...
> **对应手册**：§35.11.10 Duty cycle adjustment
...
**DCD 脚本中的值**：`0xA1390003`

```
0xA1390003 = 0b 1010_0001_0011_1001_0000_0000_0000_0011
```

| 位域 | 值 | 含义 |
|------|-----|------|
| **ZQ_EARLY_COMPARATOR_EN_TIMER [31:27]** | `10100` = 0x14 | 比较器预热时间 = **21 周期**（默认值） |
| TZQ_CS [25:23] | `011` = 3 | DDR 芯片 ZQ Short 时间 = **256 周期**（JEDEC DDR3 标准） |
| TZQ_OPEN [22:20] | `011` = 3 | DDR 芯片 ZQ Long/Oper 时间 = **256 周期**（JEDEC DDR3 标准） |
| **TZQ_INIT [19:17]** | `100` = 4 | DDR 芯片 ZQ Init（首次上电）时间 = **512 周期**（JEDEC DDR3 标准） |
| ZQ_HW_FOR [16] | 0 | 不强制立即执行（由定时器自动触发） |
| ZQ_HW_PD_RES [15:11] | 读 | 下拉电阻校准结果（校准后硬件填入） |
| ZQ_HW_PU_RES [10:6] | 读 | 上拉电阻校准结果（校准后硬件填入） |
| **ZQ_HW_PER [5:2]** | `0000` = 0 | 周期性 ZQ 校准间隔 = **每 1ms** |
| **ZQ_MODE [1:0]** | `11` = 3 | **SoC pad + DDR 芯片同时执行周期性校准 + 退出自刷新时校准** |
...
|------|------|
| 0x0 | 不执行任何 ZQ 校准 |
| 0x1 | 仅在退出自刷新时执行 ZQ Long |
| 0x2 | 仅对 DDR 芯片周期性发送 ZQ Long/Short 命令 |
| **0x3** | **对 SoC ZQ pad + DDR 芯片同时执行周期校准 + 退出自刷新时校准** |
...
| 位域 | 值 | 含义 |
|------|-----|------|
| **WL_CYC_DEL1 [26:25]** | 0 | Byte1 整周期延迟 = 0 |
| WL_HC_DEL1 [24] | 0 | Byte1 半周期延迟 = 0 |
| **WL_DL_ABS_OFFSET1 [22:16]** | 0 | Byte1 绝对微调延迟 = 0 |
| **WL_CYC_DEL0 [10:9]** | 0 | Byte0 整周期延迟 = 0 |
...
```
总延迟 = (WL_DL_ABS_OFFSET / 256) × 时钟周期 + WL_HC_DEL × 0.5周期 + WL_CYC_DEL × 1周期
```
...
| 位域 | 值 | 含义 |
|------|-----|------|
| RST_RD_FIFO [31] | 0 | 不复位 Read FIFO |
| DG_CMP_CYC [30] | 0 | 采样等待 = **16 DDR 周期** |
| **DG_DIS [29]** | 0 | **DQS Gating 使能**（不禁用） |
| **HW_DG_EN [28]** | 0 | 此处不启动自动校准（仅预设起点） |
| DG_HC_DEL1 [27:24] | 0 | Byte1 半周期延迟 = 0 |
| DG_EXT_UP [23] | 0 | 上边界使用默认判定策略 |
| DG_DL_ABS_OFFSET1 [22:16] | 0 | Byte1 绝对延迟 = 0 |
| HW_DG_ERR [12] | 0 | 无校准错误 |
...
1. 从延迟 = 0 开始，每次增加半个周期
2. 发读命令，比较读到的数据与预定义的 MPR 测试图案
3. 找到第一个匹配点（下边界）和最后一个匹配点（上边界）
4. 取平均值 `(low + high) / 2` 写入 `DG_DL_ABS_OFFSET` 和 `DG_HC_DEL`
...
| 位域 | 值 | 含义 |
|------|-----|------|
| **WR_DL_ABS_OFFSET1 [14:8]** | `0x40` = 64 | Byte1 写延迟 = 64/256 × 周期 = **1/4 周期**（0.625ns @ 400MHz） |
| **WR_DL_ABS_OFFSET0 [6:0]** | `0x40` = 64 | Byte0 写延迟 = 64/256 × 周期 = **1/4 周期** |

**与 Read 延迟线的对比**：

| 寄存器 | 方向 | 延迟对象 | 初始值 |
|--------|------|---------|--------|
| MPRDDLCTL (`0x0848`) | 读 | DQS → DQ 读数据 | `0x40404040` |
| MPWRDLCTL (`0x0850`) | 写 | DQS → DQ/DM 写数据 | `0x40404040` |
...
**DCD 脚本中的值**：`0x33333333`

```
0x33333333 = 0b 0011_0011_0011_0011_0011_0011_0011_0011
             ^^^^ ^^^^ ^^^^ ^^^^ ^^^^ ^^^^ ^^^^ ^^^^
...
| 位域 | 值 | 含义 |
|------|-----|------|
| rd_dq7_del [30:28] | `011` = 3 | DQ7 增加 **3 个延迟单元**（≈90~105ps） |
| rd_dq6_del [26:24] | `011` = 3 | DQ6 增加 3 个延迟单元 |
| rd_dq5_del [22:20] | `011` = 3 | DQ5 增加 3 个延迟单元 |
| rd_dq4_del [18:16] | `011` = 3 | DQ4 增加 3 个延迟单元 |
| rd_dq3_del [14:12] | `011` = 3 | DQ3 增加 3 个延迟单元 |
| rd_dq2_del [10:8] | `011` = 3 | DQ2 增加 3 个延迟单元 |
...
| 位域 | 值 | 含义 |
|------|-----|------|
| **wr_dm0_del [31:30]** | `11` = 3 | DM0（数据掩码）增加 **3 个延迟单元** |
| wr_dq7_del [29:28] | `11` = 3 | DQ7 增加 3 个延迟单元 |
| [27:26] 保留 | 00 | — |
| wr_dq6_del [25:24] | `11` = 3 | DQ6 增加 3 个延迟单元 |
| [23:22] 保留 | 00 | — |
| wr_dq5_del [21:20] | `11` = 3 | DQ5 增加 3 个延迟单元 |
| [19:18] 保留 | 00 | — |
| wr_dq4_del [17:16] | `11` = 3 | DQ4 增加 3 个延迟单元 |
| [15:14] 保留 | 00 | — |
| wr_dq3_del [13:12] | `11` = 3 | DQ3 增加 3 个延迟单元 |
| [11:10] 保留 | 00 | — |
| wr_dq2_del [9:8] | `11` = 3 | DQ2 增加 3 个延迟单元 |
| [7:6] 保留 | 00 | — |
| wr_dq1_del [5:4] | `11` = 3 | DQ1 增加 3 个延迟单元 |
| [3:2] 保留 | 00 | — |
| wr_dq0_del [1:0] | `11` = 3 | DQ0 增加 3 个延迟单元 |

**与 Read 版本的关键区别**：

| 特性 | Read (MPRDDQBY) | Write (MPWRDQBY) |
|------|-----------------|-------------------|
| 每 DQ 位数 | 3 位（0~7） | **2 位（0~3）** |
| DM 延迟 | 无 | **有**（wr_dm_del） |
...
**DCD 脚本中的值**：`0x00921012`

| 位域 | 值 | 含义 |
|------|-----|------|
| **RD_DQS1_FT_DCC [24:22]** | `001` | Byte1 读 DQS = 51.5% 低 / 48.5% 高 |
| RD_DQS0_FT_DCC [21:19] | `010` | Byte0 读 DQS = **50%**（默认） |
| **CK_FT1_DCC [18:16]** | `001` | DDR 时钟次级微调 = 48.5% 低 / 51.5% 高 |
| CK_FT0_DCC [14:12] | `000` | DDR 时钟初级微调 = 未配置 |
| **WR_DQS1_FT_DCC [5:3]** | `001` | Byte1 写 DQS = 51.5% 低 / 48.5% 高 |
| WR_DQS0_FT_DCC [2:0] | `010` | Byte0 写 DQS = **50%**（默认） |
...
Byte0 保持默认 50%，Byte1 调为 51.5/48.5，暗示 Byte1 PCB 走线比 Byte0 略长，需要微调补偿。
...
**DCD 脚本中的值**：`0x00000800`

| 位域 | 值 | 含义 |
|------|-----|------|
| **FRC_MSB [11]** | **1** | **强制测量**——触发完整测量过程，延迟线据此发出所需延迟值。完成后硬件自动清 0 |
| MU_UNIT_DEL_NUM [25:16] | 0 | 每周期延迟单元数（测量结果回写） |
