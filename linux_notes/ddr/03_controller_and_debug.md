...
DDR3 频率升高（800MT/s 起步），传统的 T 型分支走线产生信号反射。DDR3 改用 **Fly-by 拓扑**——命令/地址/时钟线像串糖葫芦一样依次穿过每颗 DDR 颗粒：
...
**结果**：写操作时，控制器同时发出 CLK 和 DQS。CLK 走 Fly-by 长路径，DQS 走点对点短路径。两者到达 DDR 芯片的时间不同，产生 skew。

```
CLK 路径：控制器 ── Fly-by 走线 ──▶ DDR（距离远，延迟大）
DQS 路径：控制器 ── 点对点直连 ──▶ DDR（距离近，延迟小）
```

**为什么这个 skew 很致命**：

DDR 写数据用 DQS 对齐。规范要求：**DQS 的中心要对齐 CLK 的边沿**。如果错位，DDR 用 DQS 采样 DQ 时可能采在 DQ 的跳变沿上——数据错误。
...
4. 控制器读取 DQ 反馈：
   - DQ = 0 → DQS 还没到 CLK 边沿
   - DQ = 1 → DQS 已经到了
5. 控制器逐步调整 DQS 延迟，直到找到 0→1 的跳变点
6. 该跳变点 = **DQS 和 CLK 对齐的位置**

**关键限制**：MMDC 硬件自动校准最多检测 **1 个时钟周期**的 skew。如果 Fly-by 走线太长、skew 超过 1 周期，需手动设置 `MPWLDECTRL[WL_CYC_DEL]`。

**PCB Layout 要求**：
- DDR3 尽量靠近控制器
- 计算时钟信号到最远 DDR 的飞行时间，确保 skew < 1 周期 `ddr_stress_tester` 工具在实际板子上跑出来的"最优解"。在工业级产品或宽温域（-40℃~85℃）场景下，这些参数直接决定系统是否会出现周期性死机或内存读写错误。
