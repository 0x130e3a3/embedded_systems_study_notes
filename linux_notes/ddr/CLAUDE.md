...
| 优先级 | 工具 | 适用场景 |
|--------|------|---------|
| **首选** | **Wavedrom** | 时序图、波形图、信号时序关系（DDR 中大量使用） |
| **次选** | **Mermaid** | 流程图、状态图、框图、架构图、序列图 |
| **末选** | ASCII 艺术字 | 以上两者无法表达时再使用 |

**说明**：
- 本项目的 Markdown 编辑器已安装 Wavedrom 和 Mermaid 插件，可以直接渲染
- Wavedrom 适合表达 DDR 信号时序（如 DQS/DQ 对齐、读写时序、ODT 时序等）
- Mermaid 适合表达流程关系（如初始化流程、状态机、数据通路架构等）
- 简单的表格或列表不需要转为图表
